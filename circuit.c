...
/*------------------------------------------------------------------
									 Insert Free Cell List
---------------------------------------------------------------------*/
void insert_fclist(FCLIST **head, int num, int *cut)  
{
	 FCLIST *temp,*tail;
	 
	 
	 if((temp = (FCLIST*)malloc(sizeof(FCLIST)))==NULL)
	 {
		 printf("INSER_LIST out of memory\n");
		 exit(1);
	 }
...
/*----------------------------------------------------------------
										PRINT FCLIST
 -----------------------------------------------------------------*/   
void print_fclist(FCLIST *head)   
{
	printf("\nCell\tCut\n");
	while(head!=NULL)
	 {
		 printf("%d\t%d\n",head->ID, head->cut_set);
		 head=head->next;
	 }
}
